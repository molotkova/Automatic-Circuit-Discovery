...
Examples:
  ...
  python run_experiments.py --experiment pairwise-jaccard --run-ids run1 run2 --output-dir ./my_results
    ...
    parser.add_argument(
        "--experiment",
        required=True,
        choices=["all", "logit-diff", "pairwise-jaccard", "baseline-jaccard", "baseline-logit-diff"],
...
def validate_arguments(args: argparse.Namespace) -> None:
    ...
    if args.experiment in ["baseline-jaccard", "baseline-logit-diff"] and not args.baseline_id:
    ...
    if args.experiment in ["logit-diff", "pairwise-jaccard"] and args.baseline_id:
        print(f"Warning: Experiment '{args.experiment}' does not use baseline, ignoring --baseline-id")
