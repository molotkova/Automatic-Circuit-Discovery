...
class LogitDiffAnalysis:
    ...
    def run_with_baseline(self, run_ids: List[str], baseline_run_id: str) -> ExperimentResult:
        """
        ...
        """
        if self.config.verbose:
            print(f"Running logit difference analysis with baseline {baseline_run_id}...")
        ...
        result.metadata["baseline_logit_diff"] = result.results["individual_logit_diffs"].get(baseline_run_id, 0.0)
