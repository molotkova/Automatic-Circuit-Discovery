...
class MetricComputer:
    ...
    def compute_pairwise_jaccard_indices(self, circuit_batch: CircuitBatch) -> Dict[str, Dict[str, Dict[str, float]]]:
        """
    ...
    def compute_baseline_jaccard_indices(self, baseline_run_id: str, circuit_batch: CircuitBatch) -> Dict[str, Dict[str, float]]:
        """
        ...
        if baseline_run_id not in circuit_batch.circuits:
            raise ValueError(f"Baseline run ID {baseline_run_id} not found in circuit batch")
    ...
    def compute_baseline_logit_diff_relative_change(self, baseline_run_id: str, circuit_batch: CircuitBatch) -> Dict[str, float]:
        """
        ...
        """
        if self.config.verbose:
            print(f"Computing baseline logit diff relative changes with {baseline_run_id}...")
        ...
        for run_id, circuit in circuit_batch.circuits.items():
            if run_id != baseline_run_id:
                ...
                relative_change = compute_logit_diff_relative_change(baseline_circuit, circuit)
    ...
    def compute_jaccard_statistics(self, jaccard_results: Dict[str, Dict[str, float]]) -> Dict[str, Dict[str, float]]:
        """
    ...
    def compute_pairwise_jaccard_statistics(self, pairwise_results: Dict[str, Dict[str, Dict[str, float]]]) -> Dict[str, Dict[str, float]]:
        """
    ...
    def get_metric_summary(self, metric_name: str, values: List[float]) -> Dict[str, Any]:
        """
    ...
    def compute_all_metrics(self, circuit_batch: CircuitBatch, baseline_run_id: str = None) -> Dict[str, Any]:
        """
        ...
        if baseline_run_id:
            baseline_jaccard = self.compute_baseline_jaccard_indices(baseline_run_id, circuit_batch)
            ...
            baseline_logit_diff = self.compute_baseline_logit_diff_relative_change(baseline_run_id, circuit_batch)
